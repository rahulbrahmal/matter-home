...
- **Production (always-on server `kl_2_server`)** — all hosted traffic runs here, on an always-on Mac on the home LAN; nothing production-facing runs on a developer laptop. Three launchd agents (`com.matterhome.*`): `matterserver` (matter.js controller on :5580, bound to the LAN interface), `gateway` (serves API + built SPA on :8788), and `tunnel` (Cloudflare tunnel exposing the gateway at `home.sigma-rahul.com` over HTTPS, so the PWA works away from home). Deploys arrive via the `deploy-server` workflow above; the old poll-based `deploy/auto-deploy.sh` is legacy and unused. See [AGENTS.md](AGENTS.md) for the full hosting and deploy guide.
