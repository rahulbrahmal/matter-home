...
import { view, acSetMode, isPending } from '../store.js';
...
function useClim(g) {
  const [powering, setPowering] = createSignal(false); // local: mains energising before the compressor
  const mode = () => g.mode();
  ...
  const seq = async (m) => { setPowering(true); try { await acSetMode(g.ac.id, g.powers, m); } finally { setTimeout(() => setPowering(false), 900); } };
  ...
  const label = () => { const s = st(), w = fmtW(watts());
    return s === 'powering' ? 'Powering…'
      ...
      : s === 'idle' ? `Idle${w ? ` · ${w}` : ''}` : 'Off'; };
  const pending = () => isPending(g.ac.id) || g.powers.some((p) => isPending(p.id));
